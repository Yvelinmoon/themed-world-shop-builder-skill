...
async function downloadToFile(url, destinationPath) {
  ...
  try {
    await execFileAsync("/usr/bin/convert", [tempSourcePath, "PNG32:" + destinationPath], {
      maxBuffer: 16 * 1024 * 1024,
    });
  } catch (error) {
...
async function normalizeImageToSize(sourcePath, destinationPath, width, height) {
  try {
    await execFileAsync(
      "/usr/bin/convert",
      [sourcePath, "-resize", `${width}x${height}!`, "PNG32:" + destinationPath],
      {
        maxBuffer: 16 * 1024 * 1024,
      },
    );
...
async function fitImageContainToCanvas(sourcePath, destinationPath, width, height) {
  ...
  try {
    await execFileAsync(
      "/usr/bin/convert",
      [sourcePath, "-resize", `${width}x${height}`, "PNG32:" + resizedTempPath],
      {
        maxBuffer: 16 * 1024 * 1024,
      },
    );
